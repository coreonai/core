            ...
            if let Some(op) = node.op() {
                match op {
                    ...
                    Op::Matmul(lhs, rhs) => {
                        ...
                        // PATCH (7B LoRA peak-mem probe): only materialize a
                        // gradient for an operand that actually needs one.
                        // `track_op() == is_variable || op.is_some()`, so a
                        // frozen weight leaf (neither) is skipped — otherwise
                        // candle allocates a full weight-sized grad in the
                        // GradStore for every frozen matmul, which dominates
                        // peak memory in a LoRA setup (base × ~4).
                        if lhs.track_op() {
