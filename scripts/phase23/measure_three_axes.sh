...
OUT="${2:-scratch-7b-sft/p23_si_all8}"
...
DIR=scratch-7b-sft/p23_all10_final_dir
mkdir -p "$DIR"
...
CUDA_VISIBLE_DEVICES=2 ./target/release/examples/phase23_tooluse_self_improve \
  --init-dir "$DIR" --baseline --families 3,5,8,9 --n-lo 12 --n-hi 60 \
  --out-dir scratch-7b-sft/p23_all10_targets \
  > scratch-7b-sft/p23_all10_targets.log 2>&1 &
P1=$!
...
CUDA_VISIBLE_DEVICES=3 ./target/release/examples/phase23_tooluse_self_improve \
  --init-dir "$DIR" --baseline --families 0,1,2,4,6,7 --n-lo 12 --n-hi 43 \
  --out-dir scratch-7b-sft/p23_all10_retain \
  > scratch-7b-sft/p23_all10_retain.log 2>&1 &
P2=$!
...
CUDA_VISIBLE_DEVICES=4 ./target/release/examples/phase23_python_tool_7b \
  ...
  > scratch-7b-sft/p23_all10_transfer.log 2>&1 &
P3=$!
