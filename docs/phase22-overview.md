...
| EvaluatorActor aggregate-mode pipelined verify | In `EvalSequential { aggregate: true }`, each (prompt, k) generation dispatches its verify to `tokio::task::spawn_blocking` and continues to next gen. Verifies overlap with subsequent gens. Saves ~14 min/aggregate-eval (5s/prompt × 164). G5 full sweep: ~14 GPU-h saved. | `d9834a6` |
| G1 ablation (train-steps=30) per-round | 5-seed × r=2 × gen-n=164 × train-steps=30. **4/5 seeds r=2 ≥ r=1** (vs A-batch's 0/5). Mean Δ(r=2−r=1) = +0.031 (vs A-batch's −0.081). Over-training hypothesis **CONFIRMED**. | `438ce54` |
| G1 aggregate eval | 5-seed Phase-17-aligned: mean aggregate pass@1 = **0.154 ± 0.032** vs A-batch's 0.116. Better than A but still < base 0.222. Train-steps=30 reduces but doesn't eliminate damage; other hyperparams (lr/rank/corpus) may need tuning. | (no separate commit, in G1 result doc) |
