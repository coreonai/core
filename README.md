...
| Multi-round SFT through Pekko on real MBPP-100 (cross-substrate to HumanEval) | `cargo run -p llm-actors --example phase22_mbpp_mr_sft --features cuda --release -- --rounds 2` |
| Evaluate a Stage-D-trained checkpoint with Phase 17 S6's "pass@1 raw" metric | `cargo run -p llm-actors --example phase22_humaneval_baseline --features cuda --release -- --checkpoint <merged.safetensors> --n-problems 164 --passk 10 --sequential --aggregate` |
| Verify HumanEval/MBPP completions in parallel via `VerifierActor`'s `tokio::spawn_blocking` worker pool (~8× verify speedup) | already on by default in `VerifierActor::new`; `verify_concurrency = 8`. Set via `.with_concurrency(N)` |

**176 unit tests, 37 worked examples, 11 infrastructure phases + 10 measurement phases (5–14) + 6 SFT/recipe phases (15–20) + 10 Phase-21 Pekko-bridge stages + 5 Phase-22 HumanEval/MBPP-on-Pekko stages + Stage-D G1-G9 recipe resolution. Phase 20 closed the Python-side Phase 17–20 saturation curve: r=6 SFT mean 0.581 ± 0.038 (first plateau signal Δ<σ), MBPP r=5 cross-substrate 0.541 ± 0.014, project record `seed 1` at r=6 = 0.645 (3.0× base 0.216). Phase 21 closed the supervisor-side gap: all 10 stages (A C D F B E E.next E.next.next H G) shipped → `supervisor::run_multi_round` drives the full Phase 17–20 recipe shape against Candle-native Qwen2.5-Coder-0.5B end-to-end, plus a REINFORCE Stage G with verifier-as-reward. Phase 22 (A B C D E) brings real HumanEval and MBPP benchmarks as Rust `Domain` impls and drives the full Phase 17 multi-round-SFT recipe through `supervisor::run_multi_round`. Stage B reproduces Phase 17 S6's HumanEval pass@1 = 0.216 within 1σ (0.222 aggregate at n=32×k=10) via a new `EvalSequential { aggregate }` mode; Stage E ports REINFORCE to verifier-as-reward. **Stage D's multi-round SFT opened with a −0.20 regression that turned out to be a STACK of recipe divergences from Phase 17's Python `self_improve.py`, resolved in order across G1-G9:** completion-only CE mask (`bc90db5`), cosine LR warmup (`c7a7aed`), padded `batch_size=4` SFT (`59aab8d`), fresh AdamW + non-cumulative harvest per round (`e69de7e`), and — the decisive fix — **completion truncation** (`aaf0594`): our `build_program` fed the RAW model completion (trailing test scaffolding, leaked `<|fim_middle|>` tokens, cut-off-mid-statement tails) which both failed the verifier (low harvest) and failed at eval; porting Phase 17's `truncate_completion` (`Domain::truncate_completion`) closed the gap. With the full recipe (+ `top_k=0` harvest, the best high-round-gap lever from a 5-seed ablation), the loop **reproduces Phase 17 on both benchmarks** (truncated aggregate, 5-seed): **HumanEval base 0.218 → r=2 0.436 (≈ Phase 17 0.404), MBPP base 0.201 → r=2 0.447 (≈ Phase 17 SB 0.453)**; saturation curves to r=5 reproduce the diminishing-returns→plateau SHAPE (HE plateau ~0.51 at r≈4-5, MBPP ~0.50 at r≈3) with the high-round plateau ~0.05 below Phase 17's. The residual high-round gap was investigated with five paired r=5 ablations — cumulative buffer (−0.032, ruled out), harvest temperature (−0.017, ruled out), `top_k=0` (+0.026, adopted as the best lever, ~half the gap), AdamW `weight_decay=0.01` (+0.008, ruled out), and fp16 training (NaN — Candle has no autocast/GradScaler loss scaling, so Phase 17's `torch_dtype=float16` is untestable here); only `top_k=0` helped, so the residual ~0.05 is small-factor/plateau variance, not one dominant divergence. **Every Phase 17–20 finding now has a Rust-native execution path AND a numerical reproduction** — the self-evolving training loop matches the Python reference on real benchmarks. See `docs/phase21-overview.md` and `docs/phase22-overview.md` for the single entry points. CUDA 12.5 toolchain pinning required (driver 555). Zero clippy warnings under `-D warnings`, zero fmt drift.**
...
**176 unit tests, 37 worked examples, 11 infrastructure phases + 16 measurement sessions (5–20) + 10 Phase-21 Pekko-bridge stages + 5 Phase-22 HumanEval/MBPP-on-Pekko stages (Stage D reproduces Phase 17 on both benchmarks). See the run-order list below.**
